...
p2 <- ggplot() + geom_point(data = filter(HabDF2, Include == T), aes(x = Lon, y = Lat, colour = Depth), size = 0.02) + geom_point(data = HabDF, aes(x = Lat, y = Lon), size = 0.5, shape = 3) +
	coast.poly + coast.outline + coord_quickmap(xlim, ylim) + theme(legend.position = 'top', legend.text = element_text(angle = -90), legend.title = element_blank()) + 
	scale_colour_gradient2(low = cols[1], mid = cols[2], high = cols[3], midpoint = mean(HabDF$Depth), space = "Lab", na.value = "grey50", guide = "colourbar") + ggtitle("Depth")
...
print(p1)
ggsave(file = 'Substrate.png', width = 9, height = 6)

print(p2)
ggsave(file = 'Depth.png', width = 9, height = 6)
