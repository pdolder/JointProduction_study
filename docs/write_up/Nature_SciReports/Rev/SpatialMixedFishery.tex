...
(combination of species and size class) \replaced[id=PD]{form distinct
	species-groups with common drivers of spatiotemporal distributions, and
...
\added[id=PD]{Using our general framework we identify, with relatively few
	...
	(roundfish, flatfish and deeper water species). These species-groups
	...
	how this information can be used to help spatially separate
	exploitation of the different groups of species. While more nuances
...
\subsection*{Time-varying species distributions, but stability within
	\added[id=PD]{identifiable} species-groups\\} 
...
\added[id=PD]{in species association with spatiotemporal factor coefficients
	resulting in the formation of three different species-groups} (Figure
...
deeper water species, \deleted[id=PD]{species-grouping} \added[id=PD]{forming a
	species-group} negatively associated with the second axes of Figure 2a)
and the roundfish and flatfish (\added[id=PD]{two}
species-groups\deleted[id=PD]{ing} more positively associated with the second
...
shows the interdependence within \added[id=PD]{three} species-groups of
...
species\deleted[id=PD]{ within a species-groupings} for a single year (2015;
Figure 4). \\
