...
through underlying \deleted[id=PD]{contributory} spatiotemporal factors we can
...
spatial and temporal dependence within and among species\deleted[id=PD]{
	groups} for both encounter probability and positive
...
\added[id=PD]{Using our general framework we identify, with relatively few
	...
	demersal fish community can be partitioned into three groups of species
	(roundfish, flatfish and deeper water species). These assemblages
	show common trends in spatiotemporal distributions in encounter
	probability and positive density which can be partitioned into time
	invariant ("average effect") spatial trends and time variant
	("spatiotemporal") trends. Time invariant trends may be linked to
	physical characteristics of the system including depth and predominant
	substrate type, while time varying spatial trends affect the
	distribution of species within a group in a similar way. We demonstrate
	how this information can be used to help inform spatial targeting and
	avoidance of the different groups of species. While more nuanced
	differences in spatiotemporal distributions exist within a group which
	presents a greater challenge to spatially separate catches, we show how
	this information may be utilised by mangers and fishers to match catch
	to quota in highly mixed fisheries.}

\subsection*{Common \deleted[id=PD]{average spatial patterns} driv\replaced{ers
		for groups of}{ing} species \replaced[id=PD]{determining spatial
		distributions}{associations}\\} A spatial dynamic factor
analysis \added[id=PD]{was used to} decompose\deleted[id=PD]{s} the dominant
spatial patterns driving differences in encounter probability
\added[id=PD]{(which we refer to as the spatial average - over time -
	encounter probability effect)} and positive density \added[id=PD]{(the
	spatial average - over time - positive density effect)}.  The first
...
first factor for \added[id=PD]{average} encounter probability while the other
...
\replaced[id=PD]{coefficients}{values} for the second factor indicate there are
...
\replaced[id=PD]{coefficient for}{association with} the third factor for
average encounter probability) from the rest of the species (which have a
negative \replaced[id=PD]{coefficient}{association}). Positive density is
...
\replaced[id=PD]{have negative coefficients}{are negatively associated}
...
for both \added[id=PD]{average} encounter probability (-0.85, CI = -0.88 to
-0.81; Figure S1) and \added[id=PD]{average} positive density (-0.71, CI =
-0.77 to -0.65; Figure S2). A random forest classification tree assigned 80 \%
of the variance in the first factor for \added[id=PD]{average} encounter
...
\added[id=PD]{average} positive density, the variables explained less of the
...
deeper water species, \deleted[id=PD]{assemblages} \added[id=PD]{forming an
	assemblage} negatively associated with the second axes of Figure 2)
and the roundfish and flatfish (\added[id=PD]{two}
assemblages\deleted[id=PD]{ing} more positively associated with the second
...
(the flatfish) and deeper water species\added[id=PD]{-group}. As such, it can
be predicted that higher catches of a species within a
\added[id=PD]{species-}group (e.g. cod in roundfish) would be expected when
catching another species within that \added[id=PD]{species-}group (e.g.
...
correlations found for either \added[id=PD]{spatiotemporal} encounter
...
\replaced[id=PD]{underscores}{confirms} the associations among species seen in
the factor loadings, with three distinct assemblages being
\replaced[id=PD]{identified}{present}.\\
...
roundfish with higher catches of cod \deleted[id=PD]{are} associated with
...
S9)\added[id=PD]{. The correlation coefficients were 0.60 (0.52 - 0.66) and
...
from one year to the next\replaced[id=PD]{ and}{.  This} suggests that a
positive or negative association between two species is likely to persist from
one year to the next, and that species are consistently associated with each
other in the catch. \deleted[id=PD]{The correlation coefficients were 0.60
...
maps that there are subtle differences in spatial patterns in factor loading
values from one year to the next (Figures S4 and S5), indicating changes may be
driven by temporally changing environmental factors and species behaviour.\\
...
\added[id=PD]{Further, under the landing obligation the balance of risk-reward
	...
	losses for individual trips? Set in this context, the model parameter
	estimates could be used to simulate from a distribution of catches in
	the fishery at different locations and therefore inform on the
	possibility of extreme catch events and potential consequences for
	overshooting quotas. This information may be of interest in identifying
	optimum strategies, or used in future work to model closure risks for
	fisheries operating in different locations and conditions given quota
	constraints. Such analysis on risk and decision making is likely to
	hinge on micro-level decisions by fishers and such study would be an
	interesting compliment to broader scale considerations such as those
	detailed here.} \\
