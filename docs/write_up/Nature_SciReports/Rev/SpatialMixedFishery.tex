...
stocks must be fished \added[id = CM]{at an exploitation rate corresponding to}
\deleted[id = CM]{so as to be able to produce} their Maximum Sustainable Yield
...
\deleted[id = CM]{We use f}\added[id = CM]{F}isheries-independent survey data
\added[id = CM]{are used} to characterise the spatiotemporal dynamics of key
components of a fish community by employing a geostatistical Vector
Autoregressive Spatiotemporal model (VAST). \deleted[id = CM]{We implement}
\added[id = CM]{Therein,} a factor analysis decomposition to describe trends in
...
\deleted[id = CM]{which we call}\added[id = CM]{termed} ``average'' spatial
variation) and spatiotemporal variation (9 factors) for encounter probability
and positive catch rates (\deleted[id = CM]{which we call}\added[id =
CM]{termed} ``positive density'') separately\cite{Thorson2015a}.  \deleted[id =
CM]{We use the r}\added[id = CM]{R}esultant factor
analys\replaced[id=PD]{e}{i}s \added[id = CM]{are used} to identify community
...
class as a ``species\deleted[id=PD]{ group}", and present results through
transformation of the loading matrices using PCA rotation. \added[id=JT]{We use
	this PCA rotation to visualise a reduced number of orthogonal factors
	representing average spatial variation or spatiotemporal variation
	while explaining the majority of covariation among catch rates, as well
	as the association of each species with these maps. We refer to the
...
encounter and catches, such as differences in sampling efficiency (\deleted[id
=CM]{a.k.a.}catchability), while random effects capture the spatiotemporal
...
\deleted[id = CM]{We parametrise o}\added[id = CM]{O}ur spatiotemporal model
\added[id = CM]{is parametrised} using catch data from seven
...
species \deleted[id = CM]{make up \textgreater}\added[id = CM]{comprise over}
...
\added[id=PD]{Using \deleted[id = CM]{our general framework we identify,
		with}relatively few factors\deleted[id = CM]{from} \added[id =
	CM]{in} a spatial dynamic factor analysis\deleted[id = CM]{that} the
	Celtic Sea demersal fish community can be partitioned into three
	species assemblages (roundfish, flatfish and deeper water species).
	Within these assemblages there are common trends in spatiotemporal
	distributions in encounter probability and positive density, which can
	be partitioned into time invariant (``average effect") spatial trends
	and time variant (``spatiotemporal") trends. We show through
	presentation of factor coefficients that time invariant trends may be
	linked to physical characteristics of the system including depth and
	predominant substrate type, while species loadings on to time varying
	spatial trends show \deleted[id=PD]{temporal}changes in distribution of
	species over time \replaced[id=PD]{to be similar within an
		assemblage}{are \added[id = CM]{similarly} affected\deleted[id
		= CM]{a similar way}}. We demonstrate how this information can
	be used to help inform spatial targeting and avoidance of the different
	assemblages. More nuanced differences in spatiotemporal distributions
	exist within an assemblage presenting a greater challenge to spatially
	separate catches, yet we show how this information may be utilised by
	mangers and fishers to match catch to quota in highly mixed fisheries
	through changes in gear and location fished.}

\subsection*{\deleted[id = CM]{Common\deleted[id=PD]{average spatial patterns}
		driv\replaced{ers
		for groups of}{ing} species \replaced[id=PD]{determining
		spatial distributions}{associations}} 
\added[id = CM]{Describing spatial distribution of three species assemblages}
		\\} 
A spatial dynamic factor analysis \added[id=PD]{was used to}
decompose\deleted[id=PD]{s} the dominant spatial patterns driving differences
...
clear spatial pattern can been seen both for \added[id=PD]{average} encounter
probability and \added[id=PD]{average} positive density, with a positive
...
While this exploratory factor analysis \replaced[id = CM]{models}{is modelling}
unobserved drivers of distribution, we considered what might be driving the
differences seen in the spatial factor \added[id=PD]{coefficients and species}
loadings. The first factor was highly correlated with log(depth) for both
\added[id=PD]{average} encounter probability \added[id=PD]{coefficients}
(-0.85, CI = -0.88 to -0.81; Figure S1) and \added[id=PD]{average} positive
density \added[id=PD]{coefficients} (-0.71, CI = -0.77 to -0.65; Figure S2). A
random forest classification tree assigned 80 \% of the variance in the first
factor for \added[id=PD]{average} encounter probability to depth and
...
\subsection*{\replaced[id=JT]{Species assemblages show similar spatiotemporal
...
\added[id=PD]{in species association with spatiotemporal factor coefficients
	resulting in the formation of three different assemblages} (Figure 2).
The same factors appear to drive spatiotemporal \added[id=PD]{(interannual
...
\subsection*{\replaced[id=PD]{Covariance in spatiotemporal abundance within
		species assemblages}{clusters of species show similar spatial
		\replaced[id=PD]{covariance}{patterns}}\\} 
...
th\replaced[id = CM]{a}{e}n for encounter probability, but still evident are
the strong \replaced[id=JT]{correlation}{association} among the roundfish with
higher catches of cod \deleted[id=PD]{are}
\replaced[id=JT]{correlated}{associated} with higher catches of haddock (0.58)
...
\replaced[id=PD]{\deleted[id = CM]{We sought}To understand how stable
	relationships between catches of pairs of species were from one year to
	the next\deleted[id = CM]{. To do so }, we regressed the correlation
	coefficients for}{In addition to the average spatial correlations, we
	also estimated spatiotemporal correlations \added[id=PD]{Figure S7}.
	Spatial population correlations (representing} the average spatial
correlations between pairs for species \textit{x} and species \textit{y} across
all years (Figure 3) \replaced[id=PD]{with those of}{are linearly associated
	with} the spatiotemporal population correlations, representing how
correlations between species \textit{x} and species \textit{y} change from year
to year (Figure S9)\added[id=PD]{. The correlations were 0.60 (0.52 - 0.66) and
	0.47 (0.38 - 0.55) for encounter probability and positive density
	respectively \added[id=PD]{(Figures S9a and S9b)}.} \replaced[id=PD]{
	These}{,} indicate generally predictable relationships between species
from one year to
...
\subsection*{\deleted[id = CM]{Subtle differences in distributions may be
		important to} \added[id = CM]{Potential to} separate catches
	within \replaced[id = CM]{assemblages}{groups} under the landing
	obligation\deleted[id = CM]{:}\\} The analysis shows the
interdependence within \added[id=PD]{three} assemblages of roundfish, flatfish
and deeper water species, where catching one species within the group indicates
a high probability of catching the other species\replaced[id=PD]{. This}{,
	which} has important implications for how spatial avoidance can be used
...
\added[id=PD]{species-}group for each pair of species\deleted[id=PD]{ within an
	assemblage} for a single year (2015; Figure 4). \\
...
\added[id=PD]{Further, under the landing obligation the balance of risk-reward
	...
	\added[id = CM]{the} likelihood of overshooting available quota and
	potential profit and losses for individual trips? Set in this context,
	the \deleted[id=JT]{model} parameter estimates could be used to
	simulate from a distribution of catches in the fishery at different
	locations and therefore inform on the possibility of extreme catch
	events and potential consequences for overshooting quotas. This
	information may be of interest in identifying optimum strategies, or
	used in future work to model closure risks for fisheries operating in
	different locations
...
scientific contribution to \added[id=PD]{assessing the ability of technical
	measures \replaced[id = CM]{to}{for}} meet\deleted[id = CM]{ing} the
...
propose that the \added[id = CM]{dimension-reduction} framework presented in
Figures 1-4 provides a viable route to reducing the complexity of highly mixed
\replaced[id = CM]{fisheries}{systems}.  This can allow informed management
...
associated through \replaced[id = CM]{loadings}{a function} for each factor
that return\deleted[id = CM]{s} a positive or negative association of one or
