...
stocks must be fished \added[id = CM]{at an exploitation rate corresponding to} \deleted[id = CM]{so as to be able to produce} their Maximum Sustainable
...
contribute towards mitigating imbalances in quota in mixed fisheries. \deleted[id = CM]{We use
f}\added[id = CM]{F}isheries-independent survey data \added[id = CM]{are used} to characterise the spatiotemporal dynamics
of key components of a fish community by employing a geostatistical Vector
Autoregressive Spatiotemporal model (VAST). \deleted[id = CM]{We implement} \added[id = CM]{Therein,} a factor analysis
...
spatial variation (9 factors; \deleted[id = CM]{which we call}\added[id = CM]{termed} ``average'' spatial variation) and
spatiotemporal variation (9 factors) for encounter probability and positive
catch rates (\deleted[id = CM]{which we call}\added[id = CM]{termed} ``positive density'') separately\cite{Thorson2015a}.
\deleted[id = CM]{We use the r}\added[id = CM]{R}esultant factor
analys\replaced[id=PD]{e}{i}s \added[id = CM]{are used} to identify community dynamics and drivers
common among 9 species, each analysed separately for juveniles and adult
stages.  We refer to each combination of species and size class as a ``species\deleted[id=PD]{ group}", and present results through transformation of
...
encounter and catches, such as differences in sampling efficiency (\deleted[id =CM]{a.k.a.}catchability), while random effects capture the spatiotemporal dynamics of the
fish community.\\
...
\deleted[id = CM]{We parametrise o}\added[id = CM]{O}ur spatiotemporal model \added[id = CM]{is parametrised} using catch data from seven
...
species \deleted[id = CM]{make up \textgreater}\added[id = CM]{comprise over} 60 \% of landings by towed fishing gears for the
...
\added[id=PD]{Using \deleted[id = CM]{our general framework we identify, with}relatively few
	factors\deleted[id = CM]{from} \added[id = CM]{in} a spatial dynamic factor analysis,\deleted[id = CM]{that} the Celtic Sea
	...
	\added[id = CM]{similarly} affected\deleted[id = CM]{a similar way}. We demonstrate how this information can be used
	...
	distributions exist within an assemblage, presenting a greater
...
\subsection*{\deleted[id = CM]{Common\deleted[id=PD]{average spatial patterns} driv\replaced{ers
		for groups of}{ing} species \replaced[id=PD]{determining
		spatial distributions}{associations}} 
		\added[id = CM]{Spatial distribution of three species assemblages}
		\\} A spatial dynamic
...
towards the north while adult monkfish (\emph{L. budgessa} and \emph{L.
	piscatorius}), adult haddock, megrim\deleted[id=CM]{s}, juvenile plaice and sole
\replaced[id=PD]{have negative loadings}{are negatively associated} reflecting
their more southerly distribution (Figure 1).\\

While this exploratory factor analysis \replaced[id = CM]{models}{is modelling} unobserved drivers of
...
fewer significant positive or negative relationships among species th\replaced[id = CM]{a}{e}n for
...
\replaced[id=PD]{\deleted[id = CM]{We sought}To understand how stable relationships between
	catches of pairs of species were from one year to the next\deleted[id = CM]{. To do so }, we
...
\subsection*{\deleted[id = CM]{Subtle differences in distributions may be important to} \added[id = CM]{Potential to} separate
	catches within \replaced[id = CM]{assemblages}{groups} under the landing obligation\deleted[id = CM]{:}\\} The analysis
...
anglerfishes and megrim\deleted[id=CM]{s}. The offshore area has a higher contribution of
...
\added[id=PD]{Further, under the landing obligation the balance of risk-reward
	...
	\added[id = CM]{the} likelihood of overshooting available quota and potential profit and
...
scientific contribution to \added[id=PD]{assessing the ability of technical
	measures \replaced[id = CM]{to}{for}} meet\deleted[id = CM]{ing} the goal of maximising catches in mixed fisheries
...
propose that the \added[id = CM]{deimension-reduction} framework presented in Figures 1-4 provides a viable route to
reducing the complexity of highly mixed \replaced[id = CM]{fisheries}{systems}.  This can allow informed
...
associated through \replaced[id = CM]{loadings}{a function} for each factor that return\deleted[id = CM]{s} a positive or
