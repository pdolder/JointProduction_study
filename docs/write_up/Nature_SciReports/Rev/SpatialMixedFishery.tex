...
distribution varies inter-annually, the same species are consistently
...
\replaced[id=PD]{Integrating spatiotemporal information from across multiple
	sources}{Design of spatiotemporal management measures} requires an
...
driven by a positive \replaced[id=PD]{coefficient}{value} in the deeper
...
where they can target \added[id=PD]{or avoid} species from one year to the
...
species-group\deleted[id=PD]{ing} negatively associated with the second axes of
Figure 2a) and the roundfish and flatfish (species-group\deleted[id=PD]{ing}
...
within a \added[id=PD]{species-}group (e.g. cod in roundfish) would be expected
when catching another species within that \added[id=PD]{species-}group (e.g.
