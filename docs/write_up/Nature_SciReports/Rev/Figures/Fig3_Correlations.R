...
corrplot(COR_O1,  order="hclust" ,addCoef.col = cols_o1, method = "square",
		  ...
		  cl.pos = 'n', addrect = 3, tl.col = 'black', tl.cex = 1.5, title = '(A) Spatial Encounter probability', number.cex = 0.9)
corrplot(COR_O2,  order="hclust", addCoef.col = cols_o2, method = "square", 
	          col=rev(brewer.pal(n=8, name="RdYlBu")),mar=c(0,0,1,0), bg = 'grey90' , p.mat = P_O2, insig = 'blank', 
		  cl.pos = 'n', addrect = 3, tl.col = 'black', tl.cex = 1.5, title = '(B) Spatial Density', number.cex = 0.9)
dev.off()
