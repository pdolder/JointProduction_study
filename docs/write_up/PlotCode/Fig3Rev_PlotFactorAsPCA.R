...
fish_pics <- file.path('..','..','fish_images_rev')

img1 <- readPNG(file.path(fish_pics, "plaice.png"))
...
img3 <- readPNG(file.path(fish_pics,"had.png"))
...
img7 <- readPNG(file.path(fish_pics,"meg.png"))
meg  <- rasterGrob(img7, interpolate=T, width = 1)

img8 <- readPNG(file.path(fish_pics,"monk_bud.png"))
bud  <- rasterGrob(img8, interpolate=T, width = 1)

img9 <- readPNG(file.path(fish_pics,"monk_pisc.png"))
pisc  <- rasterGrob(img9, interpolate=T, width = 1)


#b1 +  annotation_custom(g1, xmin=-0.7, xmax=-0.6, ymin=0.2, ymax=0.3) 

ysize_adu <- dim(img1)[1]/2500
xsize_adu <- dim(img1)[2]/2500

ysize_juv <- dim(img1)[1]/5000
xsize_juv <- dim(img1)[2]/5000
...
ggsave(file =  file.path('..', 'figures', 'Figure 3Rev - PCAstyle_Plots_SpatioTemp.png'), width = 16, height = 8)
