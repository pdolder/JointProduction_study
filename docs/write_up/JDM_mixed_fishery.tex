...
\item Fisheries Resource Analysis and Monitoring Division, Northwest Fisheries Science Center, National Marine Fisheries Service, NOAA, 2725 Montlake Blvd E, Seattle,
...
subtle differences within species-groups. The results highlight both
...
the respective stock quota even if they are discarded; second, by 2020 all fish stocks must be fished so
...
panels in nets), or altering the timing and location of fishing activity
...
implementing a spatio-temporal dimension reduction method that estimates 
the likely correlation in catches for multiple species at each fishing location.  We then use the results
...
and drivers common among 9 species, each analyzed separately for juveniles and adult stages.
We refer to each combination of species and size class as a "species group", and present results through
...
for encounter probability and negative with the westerly waters. This splits the roundfish species (cod, haddock and
...
with positive values in the northerly areas. Juvenile monkfish (\emph{L.
...
towards the north while adult monkfish (\emph{L. budgessa} and \emph{L.
	piscatorius}), adult haddock, megrims, juvenile plaice and sole are
negatively associated reflecting their more southerly distribution (Figure 1).
...
year to year across all species, among species-groups there were clear
...
species-groups covary in space and time through correlations among species groups. 
...
association between two species-groups is likely to persist from one year to
...
obtained directly from commercial fishing vessels rapidly while modeling densities at small time scales (e.g., weekly). 
Short-term forecasts of distribution could inform fishing choices while also
capturing seasonal differences in distributions, akin to weather forecasting.
...
The factor analysis can summarize community dynamics and identify which species and life-stages have
similar spatio-temporal patterns. This allows inference regarding species distributions
and abundance of poorly sampled species through association with other species,
and also provides estimates of spatio-temporal correlations among species-groups
\cite{Thorson2016b}.
