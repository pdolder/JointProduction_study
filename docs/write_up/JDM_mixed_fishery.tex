...
%% We propose that spatiotemporal modelling of available data is a scientific requirement to addressing the challenges of the landings obligation. The framework we propose reduces the complexity of the system to it's major components and thus allows informed management discussion over more traditional anecdotal knowledge of single-species distribution in space and time. 
...
[NEXT TWO PARAGRAPHS CAN BE MOVED LOWER OR SUPPLEMENTARY. DISCUSS HERE TO WHICH SPECIES WERE ASSOCIATED WITH WHICH FACTORS INTRODUCED ABOVE, I.E., FIGURE 2.]In a preliminary investigation of the main observed covariates influencing the factors, the first factor was highly correlated with log(depth) for both encounter
