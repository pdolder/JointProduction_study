...
together in the same net or fishing operation \deleted{(known as a `technical
	interaction')}. If managed by individual quotas, and catches do not
...
\deleted{Societal objectives for fisheries to achieve MSY across ecosystem
	components are paralleled by} \deleted{Individual fishers goals %\added[id =
...
fishing activity\cite{vanPutten2012a}. \deleted{For example, otter trawl gears are known
...
factors) for encounter probability and positive catch rates (\deleted{which we
	call} 'positive density') separately \cite{Thorson2015a}. By
...
\deleted{While this exploratory factor analysis is modelling unobserved drivers of
	distribution,} \deleted[id = brutus]{We considered what might be driving the differences seen
...
S1) and positive density (-0.71, CI = -0.77 to -0.65; Figure S2),} \deleted{A
	random forest classification tree assigned} \deleted[id = brutus]{with 80 \% of the variance
...
\deleted[id = brutus]{It is clear that depth and to a lesser extent substrate are important
...
utility of these variables as predictors of species distributions has
been}\deleted[id = brutus]{, as
...
and deeper water species. \deleted{As such,} \deleted[id = brutus]{From this it can be predicted that
...
be explained by temperature} \deleted{is often included as a covariate in
	...
	variance in the first factor values}\deleted[id = brutus]{ (Figure S6, no correlations found
...
\deleted{In addition to the average spatial correlations, we also estimate
	...
	differences in spatial patterns in factor
