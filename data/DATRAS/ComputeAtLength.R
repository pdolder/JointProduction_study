...
DF <- HL[!is.na(HL$Wt),]
DF <- DF %>% group_by(Survey, Quarter, Country, Ship, Gear, StNo, HaulNo, Year, SpeciesName, LngtClass) %>%
	summarise(Number = sum(SubFactor * HLNoAtLngt),Kg = sum(Wt)) %>% as.data.frame()
...
## Create a haul record for each species, length class
HH <- merge(x = HH, y = expand.grid(SpeciesName = unique(DF$SpeciesName), LngtClass = unique(DF$LngtClass)))

DF2 <- full_join(x = HH, y = DF)  # keep zeros
DF2$Kg[is.na(DF2$Kg)] <- 0
DF2$Number[is.na(DF2$Number)] <- 0
...
DF <- DF2[c('Survey','Ship','StNo','HaulNo','Year','Month', 'SpeciesName','HaulLatMid','HaulLonMid','HaulDur', 'SweptArea', 'SweptAreaAdj', 'LngtClass','Number','Kg')]
...
save(DF, file = file.path('..', 'Cleaned','DATRAS_No_At_Length.RData'))
