...
## M3 = Vessel as fixed effect, habitat and depth as spp specific static cov
############################################################
## 29 April 2017
## Paul Dolder
##############################################################
...
########################
  ...
  n_x = c(20, 50, 100, 250, 500, 1000, 2000)[2] # Number of stations
...
Hab2 <- Hab2[,-grep('Coarse sediment', colnames(Hab2))] # Relative to seabed 
...
## Now to convert to X_xtp format
if(mod %in% c('M3')) {
StatCov <- outer(rep(1, l = length(unique(DF$Year))), StatCov)      
StatCov <- apply(StatCov, c(1,3), t)		       
		}
...
# 32 - vessel as fixed effect + spp specific habitat covariates
if (mod %in% c('M3')) {
 TmbData = Data_Fn("Version"=Version, "FieldConfig"=FieldConfig, "RhoConfig"=RhoConfig, "ObsModel"=ObsModel, "OverdispersionConfig" = OverdispersionConfig, "c_i"=as.numeric(Data_Geostat[,'spp'])-1, "b_i"=Data_Geostat[,'Catch_KG'], "a_i"=Data_Geostat[,'AreaSwept_km2'], "Q_ik" = Vess_Cov,"X_xtp" = StatCov, "s_i"=Data_Geostat[,'knot_i']-1, "t_iz"=as.numeric(Data_Geostat[,'Year']), "a_xl"=Spatial_List$a_xl, "MeshList"=Spatial_List$MeshList, "GridList"=Spatial_List$GridList, "Method"=Spatial_List$Method)
...
# starting pos
 ...
 # Replace the values for carlhelmar cod/bud juv  - only fix these parameters,
  ...
  # Run model
  ###########
  Opt = TMBhelper::Optimize(obj=Obj, lower=TmbList[["Lower"]], upper=TmbList[["Upper"]], getsd=TRUE, savedir=paste0('../',DateFile), bias.correct=BiasCorr, control = list(rel.tol = 1e5)) 
  Report = Obj$report()
