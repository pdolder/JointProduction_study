...
n_x = c(10, 50, 100, 250, 500, 1000, 2000)[5] # Number of stations
...
Extrapolation_List = SpatialDeltaGLMM::Prepare_Extrapolation_Data_Fn(Region=Region, strata.limits=strata.limits, observations_LL=Save$Data_Geostat[,c('Lat','Lon')], maximum_distance_from_sample = max_dist)
...
plot_grid(g1,g2,leg, ncol = 3, rel_widths =c(8,8,3),  labels = c('(a) Factor loading for spatio-temporal encounter probability',
			    '(b) Factor loading for spatio-temporal catch rates',''), hjust = -0.15, vjust = 2)
ggsave(file = 'PCAstyle_Plots_Spatial_FishPics_Leg.png', width = 16, height = 8)
