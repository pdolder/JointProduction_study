...
load(file.path('..', 'QEstimates.RData')) # Extract the 2 fixed parameter estimates
...
dplyr::filter(Qs_all, Survey == 'CARLHELMAR', spp %in% c('cod_juv', 'bud_juv'))
Qs
...
ggplot(filter(Qs_all, Param %in% c('lambda1_k','lambda2_k')), aes(x = spp, y = Estimate)) +
	...
	geom_point(data = Qs, aes(y = Estimate), colour = 'red', shape = 2) +
